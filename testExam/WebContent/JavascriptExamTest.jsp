...
<script type="text/javascript">
	...
	function numCheck01() {
		var idText = document.getElementById('fId');
		
		if(isNaN(idText.value)) {
			alert('숫자가 아닙니다.')
		} else {
			alert('숫자입니다.')
		}
	...
	function searchStr02() {
		var contentsText = document.getElementById('sContents');
		var searchText = document.getElementById('val');
		
		if(contentsText.innerHTML.indexOf(searchText.value) != -1) {
			alert(searchText.value + ' 라는 단어가 존재합니다');
		} else {
			alert(searchText.value + ' (은)는 존재하지 않습니다.');
		}
	}
	
	
// 	3번
	function popupWin03(radio) {
		
		window.open(
		"./JavascriptExamTestPopupWin.jsp",
		"radio",
		"location=location"
		);
	}
	...
	function popupWinSend03() {
		...
		var radioChk = document.getElementsByName('site');
		var Chk = true;
		
		for (var i = 0; i < radioChk.length; i++) {
			if(radioChk[i].checked == false) {
				Chk = false;
				break;
			} 
		}
...
//4번
	function allchk04(allCheckbox) {
		var checkboxList = document.getElementsByName('subject');
	...
	function guguDanView05() {
		...
		if(dan.value <= 0 || isNaN(dan.value)){
			
			return;			
		}
		
		var result = document.getElementById('guguPrint');
		
		var resultStr = '';
		
		for (var i = 1; i <= 9; i++) {
			resultStr += dan.value + '*' + i + '=' + (dan.value*i) + '<br/>';
		}
	...
	function selPage06() {
		...
		var siteSelecte = document.getElementById('siteSelecte');
		
		location.href=siteSelecte.value;
	...
	function placeholderFnc07() {
		...
		if(cry.value == '') {
			...
			newDiv.innerHTML = '마야';
			newDiv.style.border = '1px solid red6';
		} else {
			root.deleteChild(root.children[root.length-1]);
		}
...
<body>
	...
	<fieldset>
		<legend>숫자입력</legend>
		<input id="fId" type="text" name="num">
...
<!-- -----------------------------------------------------------------------------------------------------------  -->
	...
	<fieldset>
		...
		<select id="siteSelecte" name="selsite" onchange="selPage06();" style="height: 30px;">
